...
vector<BigInt> RSA::cypher(string message, BigInt e, const BigInt& n) {
	...
	for (int i = 0; i < message.size(); i++) {
		cout << "word i: " << int(message[i]) << endl;
...
void Alice(int hash_a, int hash_b, int bits) {
	cout << "Alice try to send message :'rsa cryptography'" << endl;
	...
	auto msg = rsa_A.send_message("rsa cryptography", keys_A.first.d, keys_B.second);
...
int main() {
	...
	Alice(hash_a, hash_b, bitsNum);
	auto t2 = chrono::high_resolution_clock::now();

	auto ms_int = chrono::duration_cast<chrono::milliseconds>(t2 - t1);
	cout << "Time for " << bitsNum << " bits is " << ms_int.count() << " ms" <<  endl;
	return 0;
}